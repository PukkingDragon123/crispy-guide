...
(function () {
  ...
  // ------------------------------------------------------------
  // ONE COARSE GRID FOR EVERY PERSON.
  //
  // People used to be drawn on the quarter-unit grid - one native
  // pixel - which is the finest thing the raster can hold. At that
  // size a person is a smooth shape with a hairline round it, and
  // next to a mascot built out of fat blocks they read as a different
  // game. So the whole folk rig now snaps to HALF units: two native
  // pixels a step, two native pixels of black round everything.
  // Lower resolution, thicker lines, bigger shapes.
  //
  // Nothing in this file draws with G.Rh or G.Rq directly any more.
  // It all goes through R(), which snaps both corners onto the grid,
  // so there is no such thing as a half-pixel edge on a body.
  // ------------------------------------------------------------
  const GR = 0.5;                                  // the grid: 2 native px
  ...
  // Rows are GR deep and the black round it is GR thick, so the
  // silhouette is built out of blocks you can count.
  function body(g, cx, top, h, hwOf, c, o) {
    o = o || {};
    const T = tones(c);
    const n = Math.max(3, Math.round(h / GR));
    const W = [];
    for (let i = 0; i < n; i++) W.push(Math.max(GR, q(hwOf((i + 0.5) / n))));
    cx = q(cx); top = q(top);
    for (let i = 0; i < n; i++) R(g, cx - W[i] - GR, top + i * GR, W[i] * 2 + GR * 2, GR, OUT);
    R(g, cx - W[0], top - GR, W[0] * 2, GR, OUT);
    R(g, cx - W[n - 1], top + n * GR, W[n - 1] * 2, GR, OUT);
    for (let i = 0; i < n; i++) {
      const p = (i + 0.5) / n, y = top + i * GR;
      R(g, cx - W[i], y, W[i] * 2, GR, band(T, p));
      R(g, cx - W[i], y, GR, GR, T.hi);                     // rim, light side
      R(g, cx + W[i] - GR, y, GR, GR, T.dk2);               // rim, shadow side
      // the specular streak down the light shoulder
      if (!o.flat && p > 0.17 && p < 0.31 && W[i] > 2)
        R(g, cx - W[i] * 0.58, y, GR, GR, T.hi);
    }
  }
  ...
  function ball(g, cx, cy, r, col, out, lit) {
    const T = tones(col);
    const n = Math.max(2, Math.round(r * 2 / GR));
    const W = [];
    for (let i = 0; i < n; i++) {
      const d = (i + 0.5) / n * 2 - 1;
      W.push(Math.max(GR, q(r * Math.sqrt(Math.max(0, 1 - d * d)))));
    }
    ...
    if (lit && r >= 1.5) R(g, cx - r * 0.42, top + r * 0.5, GR * 2, GR * 2,
  ...
  function noodle(g, x0, y0, x1, y1, w0, w1, col, lit) {
    const T = tones(col);
    const d = Math.max(2, Math.round(Math.hypot(x1 - x0, y1 - y0) / GR));
    const P = [];
    for (let i = 0; i <= d; i++) {
      const p = i / d;
      P.push([q(G.lerp(x0, x1, p)), q(G.lerp(y0, y1, p)), Math.max(GR * 3, q(G.lerp(w0, w1, p)))]);
    }
    for (const s of P) R(g, s[0] - s[2] / 2 - GR, s[1] - s[2] / 2 - GR, s[2] + GR * 2, s[2] + GR * 2, OUT);
    for (let i = 0; i < P.length; i++) {
      const s = P[i], p = i / P.length;
      R(g, s[0] - s[2] / 2, s[1] - s[2] / 2, s[2], s[2], p < 0.16 ? T.lit : T.base);
      R(g, s[0] - s[2] / 2, s[1] - s[2] / 2, GR, s[2], lit || T.hi);
      R(g, s[0] + s[2] / 2 - GR, s[1] - s[2] / 2, GR, s[2], T.dk2);
    }
  }
  ...
  function hem(g, x, y, w, c) {
    R(g, x, y, w, GR * 2, G.shade(c, -0.3));
    R(g, x, y, w, GR, G.shade(c, 0.3));
  }
  function stitch(g, x, y, w, c) {
    for (let i = 0; i < w; i += 2) R(g, x + i, y, GR, GR, c);
  }
  ...
  G.drawFolk = function (g, x, footY, scale, o) {
    ...
    for (const [key, sd] of [['legL', -1], ['legR', 1]]) {
      ...
      if (bare && !gene.shorts) {                     // a sock over the shin
        R(g, q(fx - tot * 0.05), q(fy - shoeH - tot * 0.05), q(tot * 0.1), q(tot * 0.05), '#f2ece0');
      }
      ...
      R(g, sx + 0.5, sy, sw2 - 1, 0.25, G.shade(gene.shoe, 0.4));
      R(g, sx + 0.75, sy + 0.25, sw2 * 0.34, 0.25, G.shade(gene.shoe, 0.6));
      ball(g, sx + sw2 * 0.82, sy + sh2 * 0.45, sh2 * 0.5, gene.shoe, null, 1);
      R(g, sx, sy + sh2 - 0.75, sw2, 0.75, G.shade(gene.shoe, -0.5));
      R(g, sx, sy + sh2 - 1, sw2, 0.25, '#e8e2d6');
      R(g, sx + sw2 * 0.3, sy + sh2 * 0.34, 1, 1, '#f0e8dc');
      R(g, sx + sw2 * 0.46, sy + sh2 * 0.34, 1, 1, '#f0e8dc');
      if (A.step && sd * A.step > 0.86 && lift < 1) {
        g.globalAlpha = 0.3;
        for (let k = 0; k < 3; k++) R(g, fx - shoeW * 0.5 - k * 1.5, fy - 0.5 - k * 0.5, 2, 1, '#a89882');
        g.globalAlpha = 1;
      }
    }
    ...
    if (K === 'shirt') {
      for (let i = 0; i < 4; i++) R(g, cxT - 0.5, torT + torHn * (0.25 + i * 0.17), 1, 1, G.shade(top, -0.5));
      R(g, cxT - tW * 0.55, torT + torHn * 0.04, tW * 0.5, 0.5, '#f6f2e6');
      R(g, cxT + tW * 0.05, torT + torHn * 0.04, tW * 0.5, 0.5, '#f6f2e6');
    } else if (K === 'stripe') {
      for (let i = 0; i < 7; i++)
        R(g, cxT - tW * 0.94, torT + torHn * (0.1 + i * 0.12), tW * 1.88, Math.max(0.5, torHn * 0.055), gene.top2);
    } else if (K === 'vest') {
      R(g, cxT - tW * 0.62, torT, tW * 0.34, torHn * 0.5, gene.top2);
      R(g, cxT + tW * 0.28, torT, tW * 0.34, torHn * 0.5, gene.top2);
    } else if (K === 'hoodie') {
      R(g, cxT - tW * 0.8, torT + torHn * 0.5, tW * 1.6, Math.max(1, torHn * 0.22), topD);
      R(g, cxT - tW * 0.2, torT, tW * 0.4, torHn * 0.16, G.shade(top, -0.55));
    } else if (K === 'coat') {
      R(g, cxT - 0.25, torT + torHn * 0.1, 0.5, torHn * 0.9, topD);
      R(g, cxT - tW * 0.8, torT + torHn * 0.02, tW * 0.5, torHn * 0.3, topL);
      R(g, cxT + tW * 0.3, torT + torHn * 0.02, tW * 0.5, torHn * 0.3, topL);
      R(g, cxT - tW, hipY - 0.5, tW * 2, 1, topD);
    } else if (K === 'dress') {
      const hemY = hipY + legH * 0.22, hemY0 = torHn;
      body(g, cxT, hipY - hemY0 * 0.1, hemY - hipY + hemY0 * 0.1,
        (p) => G.lerp(hipW * 0.5, hipW * 0.86, p), top);
      for (let i = 0; i < 5; i++)
        R(g, cxT - hipW * 0.4 + i * hipW * 0.2, hemY - torHn * 0.1, 0.25, torHn * 0.1, topD);
      hem(g, cxT - hipW * 0.43, hemY - 0.5, hipW * 0.86, top);
    } else if (K === 'cardi') {
      // knitted, worn open, one button done up out of habit
      R(g, cxT - tW * 0.34, torT + torHn * 0.06, tW * 0.68, torHn * 0.94, gene.top2);
      for (let i = 0; i < 9; i++)
        R(g, cxT - tW * 0.34, torT + torHn * (0.08 + i * 0.1), tW * 0.68, 0.25, G.shade(gene.top2, -0.14));
      R(g, cxT - tW * 0.38, torT + torHn * 0.04, 0.5, torHn * 0.94, topD);
      R(g, cxT + tW * 0.34, torT + torHn * 0.04, 0.5, torHn * 0.94, topD);
      R(g, cxT - tW * 0.58, torT + torHn * 0.02, tW * 0.24, torHn * 0.16, topL);
      R(g, cxT + tW * 0.34, torT + torHn * 0.02, tW * 0.24, torHn * 0.16, topL);
      R(g, cxT + tW * 0.28, torT + torHn * 0.4, 1, 1, '#f6f2e6');
      for (let i = 0; i < 4; i++)                     // the cable knit down each side
        R(g, cxT - tW * 0.62, torT + torHn * (0.24 + i * 0.16), tW * 0.2, 0.25, topL);
    } else if (K === 'jumper') {
      R(g, cxT - tW * 0.9, torT + torHn * 0.02, tW * 1.8, Math.max(0.5, torHn * 0.08), topL);
      for (let i = 0; i < 3; i++)
        R(g, cxT - tW * 0.5 + i * tW * 0.5 - 0.5, torT + torHn * 0.4, 1, 1, gene.top2);
    }
    ...
    const colW = tW * 0.5;
    R(g, q(cxT - colW * 0.5), q(torT + torHn * 0.02), q(colW), 0.5, G.shade(top, 0.4));
    g.globalAlpha = 0.3;
    R(g, q(cxT - colW * 0.55), q(torT + torHn * 0.06), q(colW * 1.1), 0.5, '#000000');
    g.globalAlpha = 1;
    if (gene.scarf) {                                  // and a scarf, on some
      const sc2 = gene.top2;
      R(g, q(cxT - tW * 0.72), q(torT + torHn * 0.04) - 0.25, q(tW * 1.44), q(torHn * 0.16) + 0.5, OUT);
      R(g, q(cxT - tW * 0.7), q(torT + torHn * 0.04), q(tW * 1.4), q(torHn * 0.14), sc2);
      R(g, q(cxT - tW * 0.7), q(torT + torHn * 0.04), q(tW * 1.4), 0.25, G.shade(sc2, 0.4));
      R(g, q(cxT + tW * 0.22) - 0.25, q(torT + torHn * 0.16) - 0.25, q(tW * 0.3) + 0.5, q(torHn * 0.3) + 0.5, OUT);
      R(g, q(cxT + tW * 0.22), q(torT + torHn * 0.16), q(tW * 0.3), q(torHn * 0.3), G.shade(sc2, -0.12));
      for (let i = 0; i < 4; i++)
        R(g, cxT + tW * 0.24 + i * 1.5, torT + torHn * 0.44, 1, 1, G.shade(sc2, -0.4));
    }
    if (o.onTorso) o.onTorso(g, { cxT, torT, torHn, shW, hipW, tW, tot, hipY, legH, torHw });
    if (o.badge) {                                     // a name badge, for staff
      R(g, cxT + tW * 0.3, torT + torHn * 0.2, tW * 0.5, torHn * 0.16, '#f2ece0');
      R(g, cxT + tW * 0.34, torT + torHn * 0.24, tW * 0.42, 0.25, '#8a94a8');
    }
    ...
    for (const [key, sd] of [['armL', -1], ['armR', 1]]) {
      ...
      if (cuffX !== null) {
        R(g, q(cuffX - w1 * 0.58), q(cuffY - w1 * 0.5), q(w1 * 1.16), 0.5, G.shade(top, 0.4));
        R(g, q(cuffX - w1 * 0.58), q(cuffY), q(w1 * 1.16), 0.25, G.shade(top, -0.4));
      }
    ...
    if (nkH > 0.4) {
      R(g, q(hxc - tot * 0.035) - 0.25, q(nkY - 0.25), q(tot * 0.07) + 0.5, q(nkH) + 0.75, OUT);
      R(g, q(hxc - tot * 0.035), q(nkY), q(tot * 0.07), q(nkH) + 0.5, G.mix(skin, '#5a3a2a', 0.4));
    }
    ...
    for (const sd of [-1, 1]) {
      ...
      const ey = eyY + eR * 0.4;
      if (dead) { R(g, q(ex - eR), q(ey), q(eR * 2), 0.25, skinD); continue; }
      if (A.blink && !o.noBlink) {
        // a shut lid, with a lash under it
        R(g, q(ex - eR), q(ey - eR * 0.1), q(eR * 2), 0.5, '#3a2a30');
        R(g, q(ex - eR * 0.7), q(ey + eR * 0.4), q(eR * 1.4), 0.25, skinD);
        continue;
      }
      ...
      R(g, ex - r * 0.34, ey - r * 0.4, 1, 1, '#ffffff');    // the pip
      if (r > 1.6) R(g, ex + r * 0.28, ey + r * 0.24, 1, 1, '#6b5f78');
      ...
      if (Math.abs(gene.brow) > 0.25 || A.stare) {
        ...
        R(g, q(ex - eR * 0.9), q(ey - r - eR * 0.85 + lift + sd * gene.brow * eR * 0.35),
    ...
    if (gene.freck) for (let i = 0; i < 6; i++) {
      const sd = i < 3 ? -1 : 1;
      R(g, hcx + sd * hdW * (0.2 + (i % 3) * 0.09) + turn * 0.5, eyY + eR * (1.8 + (i % 2) * 0.5), 1, 1, skinD);
    }
    // ---- the nose ----
    ...
    if (gene.nose === 'button') ball(g, nX, nY, hdH * 0.06, G.mix(skin, '#e79a90', 0.45), skinD, G.shade(skin, 0.4));
      ...
      for (let i = 0; i < n2; i++) R(g, q(nX - 0.5), q(nY - hdH * 0.08 + i * 0.25), q(0.5 + i * 0.25) + 0.5, 0.25, OUT);
      for (let i = 0; i < n2; i++) R(g, q(nX - 0.25), q(nY - hdH * 0.08 + i * 0.25), q(0.25 + i * 0.25), 0.25, skinD);
    } else if (gene.nose === 'spud') { ball(g, nX, nY, hdH * 0.11, skinD, OUT); ball(g, nX, nY - hdH * 0.02, hdH * 0.07, skin, null); }
    else if (gene.nose === 'ski') {
      for (let i = 0; i < 4; i++) R(g, q(nX - 0.5 + i * 0.25), q(nY - hdH * 0.1 + i * hdH * 0.05), 1.25, 0.5, skinD);
      R(g, q(nX + 0.25), q(nY + hdH * 0.05), 0.75, 0.25, OUT);
    } else { R(g, q(nX - hdH * 0.09), q(nY), q(hdH * 0.18), 0.5, skinD); }
    ...
    if (dead) {
      R(g, q(mx0 - mW * 0.5), q(mY), q(mW), 0.5, skinD);
      ...
      if (gene.teeth) R(g, q(mx0 - ow * 0.38), q(mY), q(ow * 0.76), 0.5, '#fbf8f2');
      R(g, q(mx0 - ow * 0.2), q(mY + oh - 0.5), q(ow * 0.4), 0.25, '#c0546a');
      R(g, mx0 - ow * 0.5 - 0.75, mY - 0.5, 1, 1, '#a04a62');
      R(g, mx0 + ow * 0.5 - 0.25, mY - 0.5, 1, 1, '#a04a62');
    } else {
      ...
      for (let i = 0; i < n2; i++) {
        ...
        R(g, q(mx0 - mW * wideS * 0.5 + p2 * mW * wideS), q(yy), 0.25, 0.5, '#a04a62');
        R(g, q(mx0 - mW * wideS * 0.5 + p2 * mW * wideS), q(yy + 0.5), 0.25, 0.25, '#ffd8e0');
      }
      // the ends turn up, which is the entire expression
      R(g, mx0 - mW * wideS * 0.5 - 0.5, mY - 0.5, 1, 1, '#a04a62');
      R(g, mx0 + mW * wideS * 0.5 - 0.25, mY - 0.5, 1, 1, '#a04a62');
      if (gene.teeth && (o.smile || A.hold))
        R(g, q(mx0 - mW * 0.3), q(mY + dip * 0.5), q(mW * 0.6), 0.25, '#fbf8f2');
    }
  ...
  function prop(g, kind, h, tot, gene, t) {
    ...
    if (kind === 'bag') {
      ...
      R(g, x - w / 2 - 0.25, y + u(0.03) - 0.25, w + 0.5, hh + 0.5, OUT);
      R(g, x - w / 2, y + u(0.03), w, hh, gene.top2);
      R(g, x - w / 2, y + u(0.03), w, 0.5, G.shade(gene.top2, 0.4));
      R(g, x - w / 2, y + u(0.03) + hh - 0.5, w, 0.5, G.shade(gene.top2, -0.4));
      for (let i = 0; i < 3; i++)                          // a handle over the top
        R(g, x - w * 0.3 + i * w * 0.3, y + u(0.01), 1, 1, G.shade(gene.top2, -0.5));
      ...
      for (let j = 0; j < hh; j += 0.25) {
        ...
        R(g, x - hw - 0.25, y + j, hw * 2 + 0.5, 0.25, OUT);
        R(g, x - hw, y + j, hw * 2, 0.25, j < 0.75 ? '#fbf4e8' : '#e4d6c2');
      }
      R(g, x - w * 0.62, y - 0.5, w * 1.24, 0.75, '#c8505c');
      R(g, x + w * 0.1, y - u(0.05), 0.5, u(0.05), '#e8828c');
    } else if (kind === 'cone') {
      G.cone(g, x, y + u(0.11), { w: u(0.075), h: u(0.11) });
      ball(g, x, y + u(0.02), u(0.045), '#ffd9b0', OUT, 1);
    } else if (kind === 'phone') {
      const w = u(0.055), hh = u(0.09);
      R(g, x - w / 2 - 0.25, y - 0.25, w + 0.5, hh + 0.5, OUT);
      R(g, x - w / 2, y, w, hh, '#2a2a34');
      R(g, x - w / 2 + 0.25, y + 0.5, w - 0.5, hh - 1, Math.sin(t * 3) > -0.4 ? '#8fd8ff' : '#3a4a5c');
      G.glow(g, x, y + hh * 0.5, u(0.16), u(0.16), '#8fd8ff', 0.24);
    } else if (kind === 'lolly') {
      R(g, x - 0.25, y, 0.5, u(0.1), '#d8c4a0');
      ...
      for (let i = 0; i < 14; i++)
        R(g, G.lerp(x, bx, i / 13), G.lerp(y, by + u(0.08), i / 13), 1, 1, '#c8b490');
      ball(g, bx, by, u(0.075), ['#e0574a', '#3a8ac8', '#ffd45a'][Math.floor(gene.ph * 2) % 3], OUT, 1);
      R(g, bx - 0.5, by + u(0.075), 1, 1, OUT);
    } else if (kind === 'brolly') {
      R(g, x - 0.25, y - u(0.3), 0.5, u(0.34), '#4a5568');
      for (let i = 0; i < 9; i++) {
        const dx = (i - 4) * u(0.038), dip = Math.abs(i - 4) * u(0.012);
        R(g, x + dx - u(0.022), y - u(0.31) + dip - 0.25, u(0.044), 0.75, OUT);
        R(g, x + dx - u(0.022), y - u(0.31) + dip, u(0.044), 0.5, i % 2 ? gene.top2 : '#f6ead8');
      }
      R(g, x - 0.5, y - u(0.33), 1, 1, '#8a6a44');
    } else if (kind === 'flowers') {
      // a wrap in the fist, stems out of the top of it
      R(g, q(x - u(0.035)) - 0.25, q(y - u(0.02)) - 0.25, q(u(0.07)) + 0.5, q(u(0.075)) + 0.5, OUT);
      R(g, q(x - u(0.035)), q(y - u(0.02)), q(u(0.07)), q(u(0.075)), '#f6ead8');
      R(g, q(x - u(0.035)), q(y - u(0.02)), q(u(0.07)), 0.25, '#ffffff');
      for (let i = 0; i < 4; i++) {
        const lean = (i - 1.5) * u(0.022);
        const fy = y - u(0.03);
        for (let k = 0; k < 8; k++)
          R(g, q(x + lean * (k / 7)), q(fy - k * u(0.014)), 0.5, u(0.02), '#6b9a4a');
  ...
  function hairBack(g, gene, cx, top, w, h, c, turn) {
    ...
    if (s === 'bunwave') {
      ...
      for (let i = 0; i < 3; i++)
        R(g, q(cx - w * 0.12 + i * w * 0.12 + turn * 0.3), q(top - h * 0.1), 0.25, q(h * 0.1), '#b6b0a6');
      R(g, q(cx - w * 0.07 + turn * 0.3), q(top - h * 0.05), q(w * 0.14), 0.25, '#c8a24a');
      return;
    }
    if (s === 'puff') ball(g, cx, top + h * 0.32, w * 0.66, cd, OUT);
      ...
      for (let i = 0; i < 5; i++) {
        ...
        for (let j = 0; j < n; j++) {
          ...
          R(g, q(sx - hw) - 0.25, q(top + h * 0.1 - sh + j * 0.25), q(hw * 2) + 0.5, 0.25, OUT);
        }
        for (let j = 0; j < n; j++) {
          const hw = Math.max(0.25, w * 0.09 * (j / n));
          R(g, q(sx - hw), q(top + h * 0.1 - sh + j * 0.25), q(hw * 2), 0.25, cd);
        }
      }
    }
  }
  function hairFront(g, gene, cx, top, w, h, c, turn) {
    ...
    if (s === 'bunwave') {
      ...
      for (let i = 0; i < rows; i++) R(g, q(cx - W[i] + turn * 0.35) - 0.25, q(top - 0.25 + i * 0.25), W[i] * 2 + 0.5, 0.25, OUT);
      for (let i = 0; i < rows; i++)
        R(g, q(cx - W[i] + turn * 0.35), q(top - 0.25 + i * 0.25), W[i] * 2, 0.25, i < rows * 0.4 ? hi : base);
      ...
      for (let i = 0; i < 4; i++)                      // combed strands
        R(g, q(cx - w * 0.3 + i * w * 0.2 + turn * 0.35), q(top + h * 0.02), 0.25, q(h * 0.09), lo);
      for (const sd of [-1, 1])                        // the wisps she never gets in
        R(g, q(cx + sd * w * 0.45 + turn * 0.3) - (sd > 0 ? q(w * 0.08) : 0), q(top + h * 0.12), q(w * 0.08), q(h * 0.26), base);
      return;
    }
    if (s === 'bald') {
      R(g, q(cx - w * 0.2 + turn * 0.4), q(top + h * 0.1), q(w * 0.24), 0.5, G.shade(gene.skin, 0.5));
      if (gene.old) for (const sd of [-1, 1])
        R(g, q(cx + sd * w * 0.42 + turn * 0.3), q(top + h * 0.2), q(w * 0.1), q(h * 0.18), c);
      return;
    }
    if (s === 'combover') {
      for (let i = 0; i < 6; i++)
        R(g, q(cx - w * 0.42 + turn * 0.3), q(top + h * 0.08 + i * 0.5), q(w * (0.5 + i * 0.06)), 0.5, i < 2 ? cl : c);
      return;
    }
    ...
    for (let i = 0; i < n; i++) R(g, q(cx - W[i] + turn * 0.35) - 0.25, q(top - 0.25 + i * 0.25), W[i] * 2 + 0.5, 0.25, OUT);
    for (let i = 0; i < n; i++) {
      R(g, q(cx - W[i] + turn * 0.35), q(top - 0.25 + i * 0.25), W[i] * 2, 0.25, i < n * 0.3 ? cl : c);
    }
    ...
    if (s === 'mop') {
      for (let i = 0; i < 7; i++) {
        ...
        R(g, q(fx) - 0.25, q(fy) - 0.25, q(w * 0.13) + 0.5, q(fh) + 0.5, OUT);
        R(g, q(fx), q(fy), q(w * 0.13), q(fh), c);
      }
    } else if (s === 'bowl') {
      R(g, q(cx - w * 0.48 + turn * 0.35) - 0.25, q(fy) - 0.25, q(w * 0.96) + 0.5, q(h * 0.06) + 0.5, OUT);
      R(g, q(cx - w * 0.48 + turn * 0.35), q(fy), q(w * 0.96), q(h * 0.06), c);
    } else if (s === 'flat') {
      R(g, q(cx - w * 0.48 + turn * 0.35), q(fy - 0.5), q(w * 0.4), 0.5, cl);
    }
    for (const sd of [-1, 1])                            // sideburns down past the ear
      R(g, q(cx + sd * w * 0.46 + turn * 0.3) - (sd > 0 ? q(w * 0.08) : 0), q(top + h * 0.16), q(w * 0.08), q(h * 0.2), c);
  }
  function facialHair(g, gene, cx, top, w, h, c, mY, nY) {
    ...
    if (f === 'stache' || f === 'beard' || f === 'chops') {
      R(g, q(cx - w * 0.15), q(nY + h * 0.04), q(w * 0.3), q(h * 0.045), cd);
      R(g, q(cx - w * 0.19), q(nY + h * 0.05), q(w * 0.05), q(h * 0.03), cd);
      R(g, q(cx + w * 0.14), q(nY + h * 0.05), q(w * 0.05), q(h * 0.03), cd);
    }
    if (f === 'beard') {
      body(g, cx, mY + h * 0.02, h * 0.2, (p) => w * 0.3 * (1 - p * 0.45), cd);
    } else if (f === 'chops') {
      for (const sd of [-1, 1]) R(g, q(cx + sd * w * 0.36 - (sd > 0 ? q(w * 0.1) : 0)), q(top + h * 0.5), q(w * 0.1), q(h * 0.24), cd);
    } else if (f === 'stubble') {
      g.globalAlpha = 0.34;
      R(g, q(cx - w * 0.3), q(mY - h * 0.03), q(w * 0.6), q(h * 0.16), '#3a2a2a');
      g.globalAlpha = 1;
    }
  }
  function specs(g, gene, cx, ey, eyX, eR, w) {
    ...
    for (const sd of [-1, 1]) {
      ...
      if (gene.specs === 'half') {
        ...
        R(g, q(ex - lw), q(ly), q(lw * 2), 0.25, rim);            // straight top
        const rows = Math.max(2, Math.round(lh * 4));
        for (let j = 1; j <= rows; j++) {
          const hw = Math.max(0.25, q(lw * Math.sqrt(Math.max(0, 1 - Math.pow(j / rows, 2)))));
          R(g, q(ex - hw), q(ly + j * 0.25), 0.25, 0.25, rim);
          R(g, q(ex + hw - 0.25), q(ly + j * 0.25), 0.25, 0.25, rim);
          g.globalAlpha = 0.18;
          R(g, q(ex - hw), q(ly + j * 0.25), q(hw * 2), 0.25, '#cfe4ff');
          g.globalAlpha = 1;
          if (j === rows) R(g, q(ex - hw), q(ly + j * 0.25), q(hw * 2), 0.25, rim);
        }
        R(g, q(ex + sd * lw), q(ly), q(w * 0.09), 0.25, rim);     // temple arm
        continue;
      }
      ...
      if (gene.specs === 'round') {
        ...
        for (let k = 0; k < n2; k++) {
          ...
          R(g, ex + Math.cos(a2) * r, ey + eR * 0.1 + Math.sin(a2) * r, 1, 1, rim);
        }
      } else {
        R(g, q(ex - r), q(ey - r * 0.75), q(r * 2), 0.25, rim);
        R(g, q(ex - r), q(ey + r * 0.65), q(r * 2), 0.25, rim);
        R(g, q(ex - r), q(ey - r * 0.75), 0.25, q(r * 1.4), rim);
        R(g, q(ex + r - 0.25), q(ey - r * 0.75), 0.25, q(r * 1.4), rim);
      }
      // one glint across the glass, which is all you need
      R(g, ex - r * 0.45, ey - r * 0.45, 1, 1, '#ffffff');
      R(g, ex - r * 0.2, ey - r * 0.2, 1, 1, '#e8f2ff');
      R(g, q(ex + sd * r), q(ey), q(w * 0.1), 0.25, rim);
    }
    R(g, q(cx - eyX * 0.4), q(ey), q(eyX * 0.8), 0.25, rim);
  }
  function hat(g, kind, cx, top, w, h, turn, gene) {
    ...
    if (kind === 'cap') {
      ...
      R(g, q(hx - w * 0.1), q(top + h * 0.05) - 0.25, q(w * 0.62) + 0.5, q(h * 0.05) + 0.5, OUT);
      R(g, q(hx - w * 0.1), q(top + h * 0.05), q(w * 0.62), q(h * 0.05), G.shade(c, -0.2));
      ball(g, hx, top - h * 0.16, w * 0.05, G.shade(c, 0.4), null);
    } else if (kind === 'beanie') {
      const c = gene.top2;
      body(g, hx, top - h * 0.14, h * 0.3, (p) => w * 0.5 * (0.62 + p * 0.42), c);
      R(g, q(hx - w * 0.5), q(top + h * 0.08), q(w), q(h * 0.07), G.shade(c, 0.2));
      ...
      for (let i = 0; i < n; i++) {
        ...
        for (let j = 0; j < rows; j++) {
          ...
          R(g, q(sx - hw) - 0.25, q(top - sh + j * 0.25), q(hw * 2) + 0.5, 0.25, OUT);
        }
        for (let j = 0; j < rows; j++) {
          const hw = Math.max(0.25, w * 0.09 * (j / rows));
          R(g, q(sx - hw), q(top - sh + j * 0.25), q(hw * 2), 0.25, c);
        }
      }
      R(g, q(hx - w * 0.5) - 0.25, q(top - 0.25), q(w) + 0.5, q(h * 0.1) + 0.5, OUT);
      R(g, q(hx - w * 0.5), q(top), q(w), q(h * 0.1), c);
      R(g, q(hx - w * 0.5), q(top), q(w), 0.25, '#ffe6a0');
      R(g, q(hx - w * 0.1), q(top + h * 0.03), q(w * 0.2), 0.5, '#c8383a');
    } else if (kind === 'party') {
      const c = gene.top2, sh = h * 0.38;
      const rows = Math.max(3, Math.round(sh * 4));
      for (let j = 0; j < rows; j++) {
        const hw = Math.max(0.25, w * 0.3 * (j / rows));
        R(g, q(hx - hw) - 0.25, q(top - sh + j * 0.25), q(hw * 2) + 0.5, 0.25, OUT);
      }
      for (let j = 0; j < rows; j++) {
        const hw = Math.max(0.25, w * 0.3 * (j / rows));
        R(g, q(hx - hw), q(top - sh + j * 0.25), q(hw * 2), 0.25, (j >> 1) % 2 ? c : '#f6f2e6');
      }
  ...
  G.drawTracy = function (g, x, footY, scale, o) {
    ...
    const opts = Object.assign({}, o, {
      ...
      onTorso(g2, c) {
        ...
        for (let i = 0; i < rows; i++) {
          ...
          R(g2, q(c.cxT - hw), q(yy), hw * 2, 0.25, '#fbf4e8');
          for (let k = 0; k < 5; k++) {
            const sx = c.cxT - hw + hw * 0.36 * k + hw * 0.12;
            if (sx + hw * 0.12 < c.cxT + hw) R(g2, q(sx), q(yy), Math.max(0.25, q(hw * 0.14)), 0.25, '#bfe4d4');
          }
          if (i === 0 || i === rows - 1) R(g2, q(c.cxT - hw), q(yy), hw * 2, 0.25, '#e2d6c2');
        }
        const ax = c.cxT - aw * 0.5;
        // the bib strap up over one shoulder
        R(g2, q(c.cxT - c.tW * 0.44), q(c.torT + c.torHn * 0.08), 0.5, q(c.torHn * 0.3), '#fbf4e8');
        R(g2, q(c.cxT + c.tW * 0.38), q(c.torT + c.torHn * 0.08), 0.5, q(c.torHn * 0.3), '#fbf4e8');
        // waistband and a bow at the hip
        R(g2, q(ax), q(ay + ah * 0.22), q(aw), 0.75, '#e3a9bd');
        R(g2, q(ax - 0.5), q(ay + ah * 0.2), 1.25, 1.5, '#e3a9bd');
        // pocket
        R(g2, q(c.cxT + c.tW * 0.06), q(ay + ah * 0.46), q(c.tW * 0.42), q(ah * 0.34), '#f0e4d2');
        R(g2, q(c.cxT + c.tW * 0.06), q(ay + ah * 0.46), q(c.tW * 0.42), 0.25, '#d8c6ae');
        ...
        for (let i = 0; i < 4; i++)
          R(g2, q(px - c.tW * (0.14 - i * 0.035)), q(py - c.tW * 0.02 + i * 0.5),
      ...
      onHead(g2, c) {
        ...
        for (const sd of [-1, 1]) {
          ...
          for (let i = 0; i < 4; i++)
            R(g2, ex + sd * (r + 0.25 + i * 0.25), c.eyY + c.eR * (0.7 + i * 0.5), 1, 1, '#c8a24a');
          // crow's feet, because she has laughed a lot
          R(g2, ex + sd * (r + 0.5), c.eyY, 1, 1, c.skinD);
          R(g2, ex + sd * (r + 0.75), c.eyY + 0.5, 1, 1, c.skinD);
        }
        // hairpins in the bun
        R(g2, q(c.hcx - c.hdW * 0.1 + c.turn * 0.4), q(c.hyT - c.hdH * 0.14), q(c.hdW * 0.2), 0.25, '#c8a24a');
      },
    });
    ...
    if (o.torch && r.hand) {
      R(g, q(r.hand.x - r.hdW * 0.06), q(r.hand.y - r.hdH * 0.1), q(r.hdW * 0.12), q(r.hdH * 0.3), '#4a5568');
